...
void MainWindow::showRes()//显示结果
{
    if(NumOfRegex==0)
        return;
    ui->rexCont->setText("当前表达式："+content[CurIndex]);
    if(CurStatus==_NFA)
    {
        ui->NFAwidget->clear();
...
void MainWindow::on_NFAbtn_clicked()
{
    CurIndex=0;
    CurStatus=_NFA;
...
void MainWindow::on_MinDFAbtn_clicked()
{
    CurIndex=0;
    CurStatus=_MINDFA;
...
void MainWindow::on_PGbtn_clicked()
{
    CurIndex=0;
    CurStatus=_PROGRAM;
